...
class LoRAWrapper(nn.Module):
    def __init__(self, original_module, r=8, alpha=16, dropout=0.1):
        ...
        self.lora_A = nn.Parameter(torch.randn(r, self.in_features, device=device) * 0.02)
        self.lora_B = nn.Parameter(torch.zeros(self.out_features, r, device=device))
        self.lora_dropout = nn.Dropout(p=dropout).to(device)
...
def apply_lora_to_model(model, r=8, alpha=16, dropout=0.1):
    """Wrap q_proj and v_proj layers, freeze everything except LoRA."""
...
@final
class Wav2Vec2LoraAsrRecipe(TrainRecipe):
    ...
    def create_trainer(self, context: RecipeContext) -> Trainer:
        ...
        # ====================================================================
        # Apply LoRA (constant, no YAML change needed)
        # ====================================================================

        log.info("Applying LoRA to the model (q_proj, v_proj).")
        apply_lora_to_model(context.model.base_module)

        # Keep encoder frozen forever (LoRA trains only attention adapters)
        config.trainer.freeze_encoder_for_n_steps = int(1e9)
        # ====================================================================
